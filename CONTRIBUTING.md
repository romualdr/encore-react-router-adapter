...
- No JSDoc or block comments. Add a single-line comment only when the *why* is non-obvious (a workaround, an invariant, a constraint that isn't visible from the code).
...
1. Sign in at https://www.npmjs.com/ with an account that has publish rights to `encore-react-router-adapter`.
2. For the **first publish only** (before the package exists on npm), run `npm publish` locally with a 2FA-authenticated session — trusted publishers can only be configured on packages that already exist.
3. Open the package page → **Settings** → **Trusted Publisher** → **Add trusted publisher**, choose **GitHub Actions**, and fill in:
   - Organization or user: your GitHub login
   - Repository: the repo hosting this package
   - Workflow filename: `publish.yml`
   - Environment name: `npm`

**One-time setup (GitHub):**

Create a `npm` environment in **Settings → Environments** (optionally with required reviewers / branch protection on the default branch).
